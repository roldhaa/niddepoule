...
class OpenDataService {
  ...
  Future<int> ingestMontrealPotholes(int limit) async {
    ...
    try {
      ...
      final sql = 'SELECT * FROM "2cfa0e06-9be4-49a6-b7f1-ee9f2363a872" '
          'WHERE "ACTI_NOM" = \'Nid-de-poule\' '
          'AND ("DERNIER_STATUT" IS NULL OR "DERNIER_STATUT" NOT IN (\'Terminée\', \'Annulée\', \'Refusée\', \'Supprimée\')) '
          'ORDER BY "DDS_DATE_CREATION" DESC '
          'LIMIT $limit';

      final uri = Uri.parse(
        'https://donnees.montreal.ca/api/3/action/datastore_search_sql'
        '?sql=${Uri.encodeComponent(sql)}',
      );
